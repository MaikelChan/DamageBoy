...
class VRAM : IState
{
    ...
    public byte this[int index]
    {
        get
        {
            if (gameBoy.IsColorMode) return bytes[index + (DMG_SIZE * Bank) - START_ADDRESS];
            else return bytes[index - START_ADDRESS];
        }
        set
        {
            if (gameBoy.IsColorMode) bytes[index + (DMG_SIZE * Bank) - START_ADDRESS] = value;
